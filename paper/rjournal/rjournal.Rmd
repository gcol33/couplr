...
abstract: >
  ...
  the pairs and returns a certified optimum. Warm starting
...
Certificates are the check that survives leaving the test suite. `verify_assignment()` and `verify_flow()` run in $O(nm)$ and
...
The package carries 138 R test files and 37 C++
...
article. Each caches its CSV under `paper/` and re-measures only when that file
is moved aside; `paper/run_bench_suite.sh` runs them in order on one machine,
...
settled at `r share[6]` of the complete pair count, returned a certified
optimum, and ran `r sprintf("%.1f", imp_at("lazy", 50000) / imp_at("implicit", 50000))`
times faster than traversing the complete graph lazily. A sweep of
`r n_path_values` caliper values solved as one warm-started path cost a median
`r sprintf("%.2f", 1 / median(pth$wall_speedup))` of solving each point cold, and
across the `r nrow(regime)` cells of the solver-regime grid the dispatcher ran
at a median `r x(median(regime$ratio))` of the cell's fastest measured solver.
...
prunes, the prune certifies that its subtree holds no violating edge, and the
prunes together with the leaves the scan examines certify the pricing result.
