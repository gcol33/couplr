// Test suite for pure C++ Hungarian solver
...
#include "solvers/solve_hungarian_pure.h"
...
TEST_CASE("Hungarian solver - basic square matrices", "[hungarian][basic]") {
    SECTION("3x3 matrix") {
        auto cost = make_cost({
            {1, 2, 3},
            {4, 5, 6},
        ...
        auto result = lap::solve_hungarian(cost, false);
...
TEST_CASE("Hungarian solver - rectangular matrices", "[hungarian][rectangular]") {
...
TEST_CASE("Hungarian solver - maximization", "[hungarian][maximize]") {
    SECTION("3x3 maximize") {
        ...
        auto result = lap::solve_hungarian(cost, true);

        REQUIRE(result.status == "optimal");
        REQUIRE(result.total_cost == Approx(15.0));
    }

    SECTION("2x3 maximize") {
        auto cost = make_cost({
            {1, 2, 100},
            {4, 5, 6}
        });
        ...
        REQUIRE(result.status == "optimal");
        // Should pick 100 + 5 = 105 or 100 + 4 = 104
        REQUIRE(result.total_cost >= 104.0);
    }
}

TEST_CASE("Hungarian solver - forbidden edges", "[hungarian][forbidden]") {
...
TEST_CASE("Hungarian solver - empty matrix", "[hungarian][edge]") {
...
TEST_CASE("Hungarian solver - infeasible cases", "[hungarian][infeasible]") {
    SECTION("more rows than cols throws") {
        ...
        REQUIRE_THROWS_AS(lap::solve_hungarian(cost, false), lap::DimensionException);
    }
    ...
    SECTION("row with all forbidden throws") {
        ...
        REQUIRE_THROWS_AS(lap::solve_hungarian(cost, false), lap::InfeasibleException);
    }
}

TEST_CASE("Hungarian solver - assignment validity", "[hungarian][validity]") {
