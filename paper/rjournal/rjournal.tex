...
materialised matrix takes 3.17 seconds
under Jonker-Volgenant and 4.15
...
10.83x on the displaced treated group to
1.04x on the heavy-tailed cloud.
...
At 6,667 + 13,333 the dense path peaks 6256 MB
above that floor, the lazy path 22 MB and the
loop 128 MB, against
27429 MB for \CRANpkg{optmatch} and
8641 MB for \CRANpkg{MatchIt}. Two readings follow. The loop is not the smallest of the three: the lazy path
...
for 1093 MB on a matrix whose entries
...
solve peaked at 10.5, 7.2 and 8.8 times the cell bytes and the matrix at
...
\(n = 5000\) within a reasonable budget. We measured all timings in this section on 2026-09-01 under R 4.5.3 with couplr 1.7.0 at commit 3a083b1,
...
\midrule
no earlier rule applies & jv & 144 & 59 & 1.14x & 10.41x\\
no cost scale to exploit & hk01 & 45 & 34 & 1.00x & 2.18x\\
\bottomrule
\end{tabular}
\end{table}

Over the 189 cells the dispatcher picked the fastest solver of the
panel in 93 of them, at a median 1.07x of the
...
1.04x; over the 96 cells with more than
half the entries forbidden, 1.23x. Timed against the
solver each rule named rather than against the panel's best, \texttt{sap} takes a
median 1.41x of the default's time on the wide cells and \texttt{lapmod}
1.09x of it on the sparse ones, so in each case the rule was naming the
slower of the two.
...
3.92x of the fastest solver in the cell and as much as
10.41x; over the remaining 126 cells of the
rule the median is 1.06x and the worst
3.90x. The solvers that win the tied cells are
\texttt{network\_simplex} in 9 of them and
\texttt{csflow} in 5, both of them the
...
6 times faster than \CRANpkg{optmatch} at
...
\item
  ...
  Jonker-Volgenant, at a median 1.06x of the cell's
  fastest solver over most of the regime grid. Where the finite costs are
  heavily tied it takes a median 3.92x and as much as
  10.41x, behind \texttt{network\_simplex} and
  \texttt{csflow}. Fitting a rule to this grid would score it on
