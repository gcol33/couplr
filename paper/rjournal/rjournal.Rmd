...
abstract: >
  ...
  potentials the package optimises over a restricted graph: each unit
...
returns a `duals` component from `lp()` and `lp.assign()` when `compute.sens`
...
3. Dual-certified adaptive edge generation. Under
   ...
   at which point the duals bound the result's distance from the complete
...
`verify_assignment()` is what decides whether a returned matching is optimal for
...
so no unit is discarded. Every group it builds holds exactly one unit of the
smaller side and between `min_controls` and `max_controls` of the larger, which
is the variable-ratio design rather than the full matching of
@HansenKlopfer2006: that admits one-to-many and many-to-one groups in the same
solution, and an optimum over the two together can be strictly cheaper than the
best solution of either shape alone. The default solves the corresponding
...
k-to-one match, matching with replacement and a variable-ratio grouping differ
in the bounds their arcs receive, not in the code that solves them. Stating a
matching design as a minimum-cost flow problem is established
...
exact, variable-ratio grouping, and fine and refined balance. The designs that sit
...
| Variable-ratio grouping | Lower and upper capacitated circulation |
...
exceed `possible_edges` where the tree is working; what the loop saves is the
graph.
...
multipliers are exceeded on it. The matrix default bounds every one of the
sizes it is read against. The largest solve
peak, at `r mem_tight_n` units, reached `r sprintf("%.1f", mem_tight_x)` times
the cells against the `r solve_factor` the guard assumes, so at that size the
estimate came in `r sprintf("%.0f", mem_tight_mb)` MB under the peak it is meant
to bound. The guard exists to refuse a solve that will not fit rather than to
predict where a peak lands, so the multiplier has to sit above the ratios it is
read against, and at this size it does not.
...
- The dispatcher reads two properties and otherwise defaults to
  ...
  the measurements that chose it, so the default stands.
