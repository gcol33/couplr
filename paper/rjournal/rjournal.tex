...
available, \texttt{"auto"} switches to the lazy path where one exists and warns where
...
table. Over the 96 cells with at least three columns per row,
...
not fit. \texttt{estimate\_dense\_matrix\_mb()} estimates the matrix and
\texttt{estimate\_dense\_solve\_mb()} the peak of the solve, at a multiplier read off
these measurements, and \texttt{memory\_mode\ =\ "auto"} compares the second against
