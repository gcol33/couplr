...
ggplot(race_data, aes(x = reorder(algorithm, -time_ms), y = time_ms, fill = algorithm)) +
  ...
  geom_text(aes(label = paste0("(", year, ")")),
            hjust = -0.1, vjust = 2.5, size = 3.5, color = "#6c757d") +
...
ggplot(steps, aes(x = x, y = y)) +
  # Connecting arrows
  geom_segment(aes(x = 1.35, xend = 1.65, y = 1, yend = 1),
               arrow = arrow(length = unit(0.15, "cm")),
               color = "#6c757d", linewidth = 1) +
  geom_segment(aes(x = 2.35, xend = 2.65, y = 1, yend = 1),
               arrow = arrow(length = unit(0.15, "cm")),
               color = "#6c757d", linewidth = 1) +
  # Step boxes
  geom_label(aes(label = step), y = 1.35, size = 4.5, fontface = "bold",
             fill = c("#5bc0de", "#5bc0de", "#5cb85c"), color = "white",
             label.padding = unit(0.4, "lines")) +
  geom_text(aes(label = description), y = 1, size = 3.5, lineheight = 0.9) +
  geom_text(aes(label = icon), y = 0.65, size = 4, fontface = "bold",
            color = c("#6c757d", "#6c757d", "#5cb85c")) +
...
ggplot() +
  ...
  annotate("text", x = 2.5, y = 0.3, label = "W1 bids for J1: bid = (value - price) - (2nd best - price) + ε",
           size = 4, color = "#6c757d", fontface = "italic") +
...
ggplot(phases, aes(x = x, y = 1)) +
  # Connecting line
  geom_segment(aes(x = 1, xend = 5, y = 0.7, yend = 0.7),
  ...
  # Quality labels
  geom_text(aes(y = 0.7, label = quality), vjust = 2.5, size = 3.5, color = "#6c757d") +
  # Phase numbers
  geom_text(aes(y = 0.7, label = paste0("Phase ", phase)), vjust = 4.5, size = 3, color = "#aaa49b") +
  # Arrow showing direction
  annotate("segment", x = 0.5, xend = 5.5, y = 1.3, yend = 1.3,
           arrow = arrow(length = unit(0.2, "cm")), color = "#6c757d") +
  annotate("text", x = 3, y = 1.4, label = "ε shrinks, quality improves",
           size = 4, fontface = "italic", color = "#6c757d") +
  scale_size_identity() +
...
ggplot() +
  ...
  annotate("segment", x = 1, xend = 8, y = 1.7, yend = 1.7,
           arrow = arrow(length = unit(0.2, "cm")), color = "#6c757d", linewidth = 1) +
