...
smaller side and between \texttt{min\_controls} and \texttt{max\_controls} of the larger, which
is the variable-ratio design rather than the full matching of
\citet{HansenKlopfer2006}: that admits one-to-many and many-to-one groups in the same
...
10.83x on shifted to
1.04x on heavy\_tailed.
...
10.5, 7.2 and 8.8 times the cell bytes and the matrix at 1.8, 1.7 and 1.5
times them. Both ratios are read from 5,000
...
peak, at 5,000 units, reached 10.5 times
the cells against the 10 the guard assumes, so at that size the
estimate came in 20 MB under the peak it is meant
...
\(n = 5000\) within a reasonable budget. All timings in this section were measured on 2026-09-01 under R 4.5.3 with couplr 1.7.0 at commit 3a083b1,
...
settled at 0.29\% of the complete pair count and returned a certified
optimum, and over the 189 cells of the regime grid the dispatcher
ran at a median 1.07x of the cell's fastest solver.
