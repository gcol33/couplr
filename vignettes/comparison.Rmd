...
# NSW treatment group (randomized)
nsw_treat <- tibble(
  id = 1:185,
  age = pmax(17, rnorm(185, 25, 7)),
  education = pmax(0, pmin(16, rnorm(185, 10, 2))),
  black = rbinom(185, 1, 0.84),
  hispanic = rbinom(185, 1, 0.06),
  married = rbinom(185, 1, 0.19),
  nodegree = rbinom(185, 1, 0.71),
  re74 = pmax(0, rnorm(185, 2100, 5000)),
  re75 = pmax(0, rnorm(185, 1500, 3500)),
  group = "treatment"
)
...
cps_control <- tibble(
  id = 186:16000,
  age = pmax(17, rnorm(15815, 33, 11)),
  education = pmax(0, pmin(16, rnorm(15815, 12, 3))),
  black = rbinom(15815, 1, 0.07),
  hispanic = rbinom(15815, 1, 0.07),
  married = rbinom(15815, 1, 0.71),
  nodegree = rbinom(15815, 1, 0.30),
  re74 = pmax(0, rnorm(15815, 14000, 9000)),
  re75 = pmax(0, rnorm(15815, 13500, 9000)),
  group = "control"
)
...
### Challenge: Massive Control Pool

With 185 treatment vs 15,815 control units, we need efficient matching. couplr handles this with greedy matching:
...
For comparison, this scenario would require full distance matrix computation (185 × 15,815 = 2.9M entries) for optimal matching, but greedy matching finds excellent matches in milliseconds.
