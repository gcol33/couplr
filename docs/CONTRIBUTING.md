...
This document provides guidelines for contributing to couplr—its
...
[README](https://gillescolling.com/couplr/README.md).

### Obtaining the source

Clone the couplr repository:
...
couplr uses **testthat** for testing. All tests are located in
`tests/testthat/`.
...
## Project organization
    ...
    └── couplr.Rcheck/          <- Artifacts from local package checks (ignored)
...
- **Export pattern**: Do NOT put `[[Rcpp::export]]` in subdirectory
  files. Instead, create `_impl` functions in subdirectories and add
  exported wrappers in `src/rcpp_interface.cpp`.
...
vs. actual results - Method/algorithm used if relevant - R and operating
system version - Toolchain info if relevant (e.g., Rtools on Windows)

------------------------------------------------------------------------

By contributing to couplr, you agree that your code is released under
