---
title: "couplr: Optimal Matching with Verifiable Certificates and Column Generation"
date: "2026-09-02"
abstract: >
  ...
  potentials the package optimises over a restricted graph. Each unit starts with its nearest admissible partners, the omitted pairs are priced
  against the potentials the restricted solve returns, and pairs enter until
  none prices in, at which point those potentials bound how far the result can
  sit from the complete problem's optimum. Matching 16,667 treated units to
  33,333 controls holds 0.29 percent of the pairs and returns an optimum
  certified within that bound. Warm
  starting the same loop traces a matching path over a swept constraint at
  roughly half the cost of solving each point independently. A data-frame
  interface, nineteen assignment algorithms, balance diagnostics and
  cardinality matching share the same core. couplr is available on CRAN.
draft: false
author:
  - name: Gilles Colling
...
output:
  rjtools::rjournal_article:
    ...
    toc: no
header-includes:
  - "\\hypersetup{pdftitle={couplr: Optimal Matching with Verifiable Certificates and Column Generation}, pdfauthor={Gilles Colling}}"
  - \usepackage{placeins}
  - \usepackage{float}
...
full matching to \CRANpkg{optmatch} [@optmatch]. It formulates those designs as minimum-cost flow problems [@HansenKlopfer2006], reaches RELAX-IV
...
packages here and in the supplementary material's feature table were assessed on 2026-08-09
...
interfaces return material for that test in their own terms. \CRANpkg{lpSolve} returns a `duals` component from `lp()` and `lp.assign()` when `compute.sens`
...
their place, an `exceedances` attribute and a solver tolerance, both described
...
3. Dual-certified adaptive edge generation. Under
   `memory_mode = "implicit"` the complete graph is never built: a
   restricted problem is solved, the omitted pairs are priced against its
   duals, violating pairs are added, and the loop repeats until none prices in. At that point the duals bound the result's distance from the complete
   problem's optimum, and certify optimality where that bound closes.
4. Warm-started matching paths. `match_path()` sweeps one constraint over
...
We arrange the article for two kinds of reader. The assignment engine and the
...
reader interested in where the certificates and the column generation come
from.
...
formulation leaves $v$ unrestricted in sign. `verify_assignment()` imposes
$v_j \le 0$ only
...
alone, tightness on matched pairs, accepts solutions that are not optimal. In our own prototyping a perfect, dual-feasible, matched-tight solution passed
...
Setting the band to zero is not the fix, because the sign of the double evaluation can itself be wrong. A reduced cost far smaller than the potentials
...
the reals, and takes the sign of that expansion [@Shewchuk1997]. A
...
Objective equality is then not a fourth measurement but a consequence. With matched arcs tight and unmatched columns free, the primal cost is the sum of
...
Euclidean distances between normal clouds gave one in none. Those potentials miss exact tightness by a relative $2 \times 10^{-16}$ to $5 \times 10^{-15}$,
...
Every solver here targets the objective of \@ref(eq:lap), so the choice among
...
supplied one where the costs are integers and a rounding of it otherwise. `verify_assignment()` is what decides whether a returned matching is optimal for
...
smallest is zero, multiplied by a scale factor and rounded. We set the factor to keep the scaled costs clear of the sentinel marking a forbidden pair and the
...
matrix would dominate the work. Bottleneck assignment minimises the largest
pair cost instead of the sum, a different objective, and is reached through
`bottleneck_assignment()` rather than through `method`.
...
certifies the matching without a second solve. Four extensions operate on the same matrix. `lap_solve_kbest()` returns the k cheapest assignments in
...
covariates to match on and returns the optimal one-to-one pairing under the requested distance. `auto_scale = TRUE` screens the covariates for degenerate
...
matching variable, so which covariate drives a poor pair is visible without recomputing anything. `m$unmatched` holds what is left over on each side and
...
under `_left` and `_right` suffixes.
 ...
 A paired difference taken on it estimates an adjusted association, and
...
signed-rank statistic on one-to-one matched pairs [@Rosenbaum2002]. It reports p-value bounds across a grid of the odds-ratio parameter $\Gamma$ so that a
...
```{r constraints-greedy, echo=FALSE}
...
produces this result; a greedy pass over the same edges matches
`r nrow(m_greedy$pairs)` units
...
expressed in standard deviations of the linear predictor. Where the constraints
admit it, `full_match()` assigns every unit to a matched group of variable size,
so no unit is discarded. A group holds either one left unit and several right
ones or one right unit and several left ones, and both shapes appear in the same
solution, which is full matching in the sense of @HansenKlopfer2006. The default
solves the corresponding
minimum-cost flow problem with Johnson potentials [@Johnson1977]; a two-pass
greedy heuristic is reached by asking for it, under `method = "greedy"`. `cem_match()` implements coarsened exact matching
...
Either way the result reports `n_matched` beside `best_possible`, the `gap`
between them, whether the answer is `certified`, and what the search
`stopped_on`. `best_possible` is the search's current upper bound on attainable
sample size, and is the largest sample the constraints admit once `certified`
is `TRUE`. A pruning loop is available as
...
`match_data()` returns data in the layout \CRANpkg{MatchIt} users expect, so downstream code keeps working. `bal.tab()` methods are registered for the
...
# Common optimisation architecture
...
makes node potentials available from all of them. The potentials are what certify a solution; a certificate is what makes it safe to optimise over a
restricted graph of pairs; and a certified restricted solve is what a warm start
resumes from:
...
their arcs receive, not in the code that solves them. Stating full matching as a
minimum-cost flow problem is established [@HansenKlopfer2006]. Here it is the one design whose flow value is not fixed in advance, since how many pair arcs a
full matching uses depends on how many groups it forms, so the source injects
the unit count and a bypass arc absorbs the rest. What the flow model
contributes is that every
...
outside it sit outside because their answer is not a flow. `cem_match()` coarsens and strata are read off directly, `subclass_match()` cuts the
...
`memory_mode = "implicit"` changes is the problem the solver ranges over: it
...
and the cost of $X$ is $\sum_i u_i + \sum_j v_j$. Feasibility for
\@ref(eq:duals) is the pair inequalities together with $v_j \le 0$, and the
sign condition constrains $(u, v)$ alone, so it holds on the complete problem
exactly as it held on the restricted one. The hypothesis supplies the pair
inequalities on every admissible pair outside $E$, so $(u, v)$ is feasible for
...
units, and every run reported there came back with $\delta = 0$. The
certificate returns that sum as `max_suboptimality`, beside
`certified_reduced_cost_floor`, the floor the pricing scan proves for every
admissible pair, which sits below the minimum it observed exactly where pruning
supplied part of the proof.
...
complete graph and pay for the rounds that got there. `max_rounds`, 60 by
default, is a guard rather than the termination argument, and a run reaching it
returns `iteration_limit` rather than a certificate.
...
priced in one round is priced again in the next, so `edges_evaluated` can
exceed `possible_edges` where the tree is working. What the loop saves is the graph.

Rows contribute at most `keep_per_row` violators each, five by default, so a
...
So the restricted solution is optimal for a problem that was never built, and
`certify = TRUE` runs that check and returns it.

Two quantities in `$search` describe what that cost. `candidate_edges` against
`possible_edges` is the graph built against the complete one that was not, and
`edges_evaluated` counts distance evaluations over all rounds.


Round count is the quantity to control, since each round prices every omitted
pair again, and it is what the seed rule of step 1 is sized for. A seed too narrow buys what it needs a round at a time, a seed too wide pays for columns
...
Constraints tighten the loop rather than complicating it: a caliper is read on
the covariates and a distance ceiling against the tree's bound, so pairs
leave the set unpriced.
...
where the two meet. Graded matching [@YuRosenbaum2022] also enlarges a sparse network
only as far as it has to. A grade is fixed before the match, while the
...
The mode is available on `match_couples()`, `assignment()`, `full_match()` and
`cardinality_match()`, and it is requested rather than selected:
`memory_mode = "auto"` does not choose it, because the worst case above is a
real one. A design other than the assignment carries column capacities above
one, so its column potentials are not the assignment duals above; its omitted
pairs are priced against the potentials themselves instead, since a pair arc
carries a reduced cost from the nodes it joins. A master short of its flow is
extended by the pairs crossing the residual cut its excess still reaches, and
when none cross, the flow placed is the maximum over every pair. How small a graph the loop
...
admissible, which is one round of the pricing loop of the previous section, where an
independent call at that value needs a full solve. One mechanism
...
direction is a correctness requirement rather than a convention. A descending sweep withdraws pairs the current matching may be standing on, which breaks
...
size long before runtime is. A 100,000 by 100,000 problem needs 80 GB as doubles; the 200,000 units it
pairs, described by 100 covariates, need 160 MB. couplr exposes this as `memory_mode`, with values `"dense"`, `"lazy"`,
...
Under `"auto"` the package decides. Below ten million cells it skips the
check, since probing free memory on every ordinary problem would be pure
overhead. That is 80 MB of cells, and an estimated solve peak of
`r format(80 * solve_factor, big.mark = ",")` MB, so the skipped case is
not free of cost; it is the size at which the probe stops paying for
itself. Above that it estimates what a dense solve peaks at
...
builds result objects and their methods. All three run against a C++ core
reached from R through \CRANpkg{Rcpp} [@Rcpp]. The object model, the C++
...
removed; the regime grid behind Table \@ref(tab:regimes) is the measurement, and
both solvers stay reachable by name.
...
Certificates are the check that survives leaving our test suite. `verify_assignment()` and `verify_flow()` run in $O(nm)$ and
...
shapes, minimisation and maximisation, and forbidden edges. Our reference is
the same `bruteforce` solver the dispatcher uses below nine units, so the
verified and the shipped path are one implementation. We check constrained
matching against an enumeration over all partial matchings of a small
instance, which supplies both halves of its lexicographic objective. We check
the paths that avoid materialising the problem against the path that materialises it. The implicit loop and the dense solve must return the same
...
asks for is a certified objective the two agree on. We report both comparisons
in the next section, on the sizes it covers.
...
article. Each caches its CSV under `paper/` and re-measures only when that file is moved aside. `paper/run_bench_suite.sh` runs them in order on one machine,
which is how we produced these numbers, and the scaling, implicit,
implicit-grid, regime and memory scripts take `--quick` for a reduced grid. The
...
problems. The dense and lazy arms are pinned to Jonker-Volgenant, so they differ only in representation. The implicit arm carries the flow model as its
...
`r sprintf("%.0f", imp_at("lazy", 50000))` for the lazy path. At the two
smallest sizes the lazy path is faster; at `r ilab[2]` it takes
...
pairing is identical unit by unit, so the loop returns our assignment rather
...
saves is not what a change of solver would give. We give the decomposition in
...
One cloud is not a characterisation, and the table above reports one. In the
supplementary material we sweep the loop around that cloud one factor at a
time. The factors are covariate counts from `r ig_dim_rng[1]` to
`r ig_dim_rng[2]`, seven point clouds, three caliper settings including the
tightest value each problem is still feasible under, and five sizes, each at
several seeds. The clouds are a plain Gaussian cloud, clustered points, a
displaced treated group, heavy-tailed coordinates, coordinates on a coarse
lattice so that distances tie, one contested core where every treated unit's
nearest controls are the same few, and a shell where every treated-to-control
distance is nearly equal. Across the `r ig_cells` cells that grid holds, the loop settled in
...
path beside the loop returned the same pairing unit by unit. The margin over the lazy path is where the clouds separate: about
`r sprintf("%.0fx", ig_fast_x)` on `r ig_fast`, and none at all on
`r ig_slow`, where the two paths are within measurement of each other.
...
and it is still several times faster than the
...
set out to avoid, and it has paid for the rounds that built it. 
...
expression, so we run each arm as a fresh R session that loads what it needs, runs
...
compares the second against available memory. Neither multiplier is fitted to one cell: across `r mem_size_list` units the
solve peaked at `r mem_solve_list` times the cell bytes and the matrix at
`r mem_matrix_list` times them, and both defaults sit above every one of those. Both ratios are read from `r format(min(mem_sizes), big.mark = ",")`
units up: at `r format(mem_small, big.mark = ",")` the matrix holds
`r sprintf("%.0f", mem_at("dense_matrix", mem_small, "matrix_mb"))` MB of cells
against a session floor near
`r sprintf("%.0f", mem_at("baseline", mem_small, "peak_rss_mb"))` MB, so what
the peak measures there is the session rather than the solve, and both multipliers are exceeded on it. The tightest solve peak, at
`r mem_tight_n` units, reached `r sprintf("%.1f", mem_tight_x)` times the cells
against the `r solve_factor` the guard assumes, leaving
`r sprintf("%.0f", mem_tight_mb)` MB of headroom.
We report every arm and size in the supplementary material.
...
The table leads with the wall clock for the whole sweep, which is the number a user meets. It carries the feasibility check, the balance reading each point
...
`r sprintf("%.3g", max(pth$worst_total_gap))` relative, so the path reproduces
the feasibility statuses, matched counts and objective values of independent
solves. Pair identities were not compared, and where a value admits more than
one optimum equal totals do not make the matchings equal.
...
figure establishes, and they fix the dispatcher's default. Each label in the
figure is a solver's `method` name: JV `jv`, SAP `sap`, SAP-D `sap_dense`,
LAPMOD `lapmod`, Auction `auction`, Auction-GS `auction_gs`, Auction-S
`auction_scaled`, CSA `csa`, CS-flow `csflow`, Push-R `push_relabel`, Cycle-C
`cycle_cancel`, Net-S `network_simplex`, SSAP-B `ssap_bucket`, Gabow-T
`gabow_tarjan`, Ramshaw `ramshaw_tarjan`, Hungarian `hungarian`, Munkres
`munkres`, HK-01 `hk01` and Brute-F `bruteforce`.


The grid behind Table \@ref(tab:regimes) is what led us to reduce the rules to three.
...
We cross eight cost regimes with six admissibility patterns and three aspect
ratios. The regimes run from uniform integers through binary, constant,
heavily tied and heavy-tailed costs to Euclidean distances over uniform and
clustered points. The patterns are the complete matrix, four unstructured
densities from 60 percent of the entries finite down to 1 percent, and four
disconnected components. The ratios run from square to one row per ten
columns. That crossing is the base tier. The large tier repeats a
subset of it at three times the size rather than the whole. It repeats the regimes and patterns where a rule that holds at 500 rows might fail at 1500, against the
...
crossing and `r sum(regime$tier == "large")` the large-tier subset. We generate each cell as
several independent instances and time each instance several times, and every
solver in the panel sees the same instance. We time the solver `"auto"` selects
as a solver of its own, so its number carries the probing pass the rules cost,
and we read it against the fastest named solver in the same cell, whose panel
holds every solver the rules can name on it.
...
cell's best time. Where a cell reads below `1.00x` the two sides are separate
timings of the same solver, and the lowest in the grid is
`r x(min(regime$ratio))`. The two rules that were
removed can be read off the same grid. Over the `r nrow(reg_rect)` cells with
...
half the entries forbidden, `r x(median(reg_sp$ratio))`. Timed against the
solver each rule named rather than against the panel's best, `sap` takes a
median `r x(h2h_rect)` of the default's time on the wide cells and `lapmod`
`r x(h2h_sp)` of it on the sparse ones, so in each case the rule was naming the
slower of the two.

Where the default is behind, it is behind in one cost regime. Over the
`r nrow(reg_tied)` cells whose finite costs are heavily tied it takes a median
`r x(median(reg_tied$ratio))` of the fastest solver in the cell and as much as
`r x(max(reg_tied$ratio))`; over the remaining `r nrow(reg_rest)` cells of the
rule the median is `r x(median(reg_rest$ratio))` and the worst
`r x(max(reg_rest$ratio))`. The solvers that win the tied cells are
`` `r names(tied_win)[1]` `` in `r as.integer(tied_win)[1]` of them and
`` `r names(tied_win)[2]` `` in `r as.integer(tied_win)[2]`, both of them the
flow formulations that Figure \@ref(fig:solver-bench) puts last on dense uniform
costs. No rule in the table reads how tied the costs are. Fitting one to this
grid would score it on the measurements that chose it, so we fixed a rule that
sends few distinct cost levels to epsilon-scaling auction before scoring it, on
a grid generated apart from this one, with the criterion stated in advance. It
was quicker on square problems and slower than the criterion allowed on
rectangular ones. A second version bounded to near-square problems, scored on a
second independent grid, was quicker in nearly every cell it fired on and still
slower than allowed in two. Neither was kept. Both grids are in the
supplementary material.
...
Marginals could still hide different assignments, so we score all three on one objective: the same
...
alternatives, a relative difference of `r sprintf("$%.0f \\times 10^{-6}$", 1e6 * abs(diff(range(lal_obj$total_cost))) / min(lal_obj$total_cost))`. That is agreement
on the objective to the precision reported here rather than a demonstration
that all three attain the same minimum. Separating those two readings is
...
`memory_mode = "dense"`. We generate each size as several independent
instances and time the three packages on each in interleaved rounds. An
instance's time is its fastest repetition, since repetitions of one solve differ
only by what else the machine was doing, and the table reports the median and
the interquartile range across instances, which is what varies between problems.
...
We compare three workflows on one task, from data frame to matched pairs, and
not three implementations of one algorithm. The packages do not state the same problem. \CRANpkg{optmatch} solves a minimum-cost flow problem,
...
The lazy path removes the memory ceiling as well. The same $n = 50{,}000$
problems under `memory_mode = "lazy"` take a median
`r sprintf("%.0f", lazy$median_s[lazy$n_total == 50000])` seconds without ever
building the distance matrix, returning the assignment the dense path returns.
At $n = 20{,}000$ the median is
`r sprintf("%.1f", lazy$median_s[lazy$n_total == 20000])` seconds against
...
All three packages offer covariate distances, propensity-score matching, exact
blocking and a cost-matrix entry point. Where they differ is in what the
assignment layer exposes. k-best assignments, a bottleneck objective, Rosenbaum
sensitivity bounds and a public dual-potential verifier are in couplr and in
neither alternative, with optmatch reporting a gap bound in place of the last of
these. couplr solves a pair match as the rectangular assignment directly, where
`optmatch::pairmatch()` and MatchIt's optimal method route it through
`fullmatch()`, and it exposes nineteen solvers where optmatch exposes five. The
comparison is of the assignment layer alone, and omits areas where the
alternatives lead, among them the breadth of designs reachable through a single
MatchIt call and the maturity of optmatch's full matching. The supplementary
material gives the full table, what each entry was read from, and the versions
assessed.

# Summary and discussion

Making the assignment problem the organising object, rather than a hidden step
inside a matching function, is what lets one package serve a causal-inference
user starting from covariates and an operations-research user starting from a
cost matrix. 



couplr has three limitations:

- `cardinality_match()` answers a moment-constrained problem by branch and
  bound under node and time limits, so a run can stop with a gap rather than a
  certificate and reports which it reached. Fine and refined balance is
  answered certified.
- The dispatcher reads two properties and otherwise defaults to
  Jonker-Volgenant, at a median `r x(median(reg_rest$ratio))` of the cell's
  fastest solver over most of the regime grid. Where the finite costs are
  heavily tied it takes a median `r x(median(reg_tied$ratio))` and as much as
  `r x(max(reg_tied$ratio))`, behind `` `r names(tied_win)[1]` `` and
  `` `r names(tied_win)[2]` ``. A rule for tied costs, fixed in advance and
  scored on two grids generated apart from this one, did not meet its stated
  criterion on either, so the default stays.
