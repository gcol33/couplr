...
# Edge data with explicit from/to names for clarity
edges <- data.frame(
  ...
  to_y = c(3, 2, 2, 3, 3, 1),
  cost = c(2, 4, 1, 3, 3, 2),
  from_node = c("S1", "S1", "S2", "S2", "S3", "S3"),
  to_node = c("T1", "T2", "T2", "T1", "T1", "T3"),
  optimal = c(TRUE, FALSE, TRUE, FALSE, FALSE, TRUE)
)

# Offset labels to avoid overlap - position along edge path
edges$label_x <- (edges$from_x + edges$to_x) / 2
edges$label_y <- (edges$from_y + edges$to_y) / 2
# Nudge crossing edges' labels to reduce overlap
edges$label_y[4] <- edges$label_y[4] + 0.15  # S2→T1
edges$label_y[5] <- edges$label_y[5] - 0.15  # S3→T1
...
ggplot() +
  ...
  # Non-optimal edge labels (show connection explicitly)
  geom_label(data = edges_nonopt,
             aes(x = label_x, y = label_y,
                 label = paste0(from_node, "\u2192", to_node, ": ", cost)),
             size = 3.5, label.padding = unit(0.2, "lines"),
