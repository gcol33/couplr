...
I am submitting "couplr: Optimal Matching with Verifiable Certificates and Sparse Edge
...
packages, which expose one algorithm each with no preprocessing, constraints or
diagnostics.
...
what these packages return. And the set of admissible pairs is materialised
...
building the graph: each unit starts with its nearest admissible partners, the
omitted pairs are priced against the duals the sparse solve returns, and pairs
...
them, the Gabow-Tarjan bit-scaling algorithm, appears not to have had a publicly
available open-source implementation in any language before this, and the paper
says so carefully and without making it the contribution.
...
it holds 0.29% of the pairs, returns the assignment the dense solve returns, and
comes back certified with a duality gap of zero, in a third of the time the
same problem takes without the loop. A caliper sweep of twenty values solved as
...
Every number in the paper is reproducible from the scripts and data files
included in the submission; the article's own code chunks run in well under a
minute, since the expensive benchmarks are shipped as CSV output from the
...
problem who currently reach for clue or lpSolve. couplr is on CRAN, has
