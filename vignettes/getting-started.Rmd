...
For datasets larger than a few thousand observations, optimal matching becomes slow. Use `match_couples(method = "greedy")` instead; it's 10-100x faster with nearly identical results:
...
One-to-one matching discards unmatched controls. If you want every unit in a group, use `full_match()`. It creates variable-ratio groups (e.g., 1 treatment + 3 controls) that minimize total distance. Every group holds exactly one unit of the smaller side, which is narrower than the full matching of Hansen and Klopfer (2006):
...
- `?match_couples` with `method = "greedy"` - Fast approximate matching
