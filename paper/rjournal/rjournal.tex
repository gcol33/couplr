...
\title{couplr: Optimal Matching with Verifiable Certificates and Column Generation}
...
full matching to \CRANpkg{optmatch} \citep{optmatch}. It formulates those designs as minimum-cost flow problems \citep{HansenKlopfer2006}, reaches RELAX-IV
...
interfaces return material for that test in their own terms. \CRANpkg{lpSolve} returns a \texttt{duals} component from \texttt{lp()} and \texttt{lp.assign()} when \texttt{compute.sens}
...
\item
  ...
  \texttt{memory\_mode\ =\ "implicit"} the complete graph is never built: a
...
\item
  Warm-started matching paths. \texttt{match\_path()} sweeps one constraint over
...
Results were produced with couplr version 1.7.1.
...
#>   max_suboptimality        1.330997e-16
...
supplied one where the costs are integers and a rounding of it otherwise. \texttt{verify\_assignment()} is what decides whether a returned matching is optimal for
...
\texttt{bottleneck\_assignment()} rather than through \texttt{method}.
...
certifies the matching without a second solve. Four extensions operate on the same matrix. \texttt{lap\_solve\_kbest()} returns the k cheapest assignments in
...
covariates to match on and returns the optimal one-to-one pairing under the requested distance. \texttt{auto\_scale\ =\ TRUE} screens the covariates for degenerate
...
matching variable, so which covariate drives a poor pair is visible without recomputing anything. \texttt{m\$unmatched} holds what is left over on each side and
...
signed-rank statistic on one-to-one matched pairs \citep{Rosenbaum2002}. It reports p-value bounds across a grid of the odds-ratio parameter \(\Gamma\) so that a
...
admit it, \texttt{full\_match()} assigns every unit to a matched group of variable size,
...
solution, which is full matching in the sense of \citet{HansenKlopfer2006}. The default
solves the corresponding
minimum-cost flow problem with Johnson potentials \citep{Johnson1977}; a two-pass
greedy heuristic is reached by asking for it, under \texttt{method\ =\ "greedy"}. \texttt{cem\_match()} implements coarsened exact matching
...
Either way the result reports \texttt{n\_matched} beside \texttt{best\_possible}, the \texttt{gap}
between them, whether the answer is \texttt{certified}, and what the search
\texttt{stopped\_on}. \texttt{best\_possible} is the search's current upper bound on attainable
sample size, and is the largest sample the constraints admit once \texttt{certified}
is \texttt{TRUE}. A pruning loop is available as
...
\texttt{match\_data()} returns data in the layout \CRANpkg{MatchIt} users expect, so downstream code keeps working. \texttt{bal.tab()} methods are registered for the
...
minimum-cost flow problem is established \citep{HansenKlopfer2006}. Here it is the one design whose flow value is not fixed in advance, since how many pair arcs a
...
outside it sit outside because their answer is not a flow. \texttt{cem\_match()} coarsens and strata are read off directly, \texttt{subclass\_match()} cuts the
...
\eqref{eq:duals} is the pair inequalities together with \(v_j \le 0\), and the
...
certificate returns that sum as \texttt{max\_suboptimality}, beside
\texttt{certified\_reduced\_cost\_floor}, the floor the pricing scan proves for every
...
exceed \texttt{possible\_edges} where the tree is working. What the loop saves is the graph.
...
The mode is available on \texttt{match\_couples()}, \texttt{assignment()}, \texttt{full\_match()} and
\texttt{cardinality\_match()}, and it is requested rather than selected:
\texttt{memory\_mode\ =\ "auto"} does not choose it, because the worst case above is a
...
pairs, described by 100 covariates, need 160 MB. couplr exposes this as \texttt{memory\_mode}, with values \texttt{"dense"}, \texttt{"lazy"},
...
Under \texttt{"auto"} the package decides. Below ten million cells it skips the
...
rather than what the matrix occupies, at 12 times the raw cell
...
reached from R through \CRANpkg{Rcpp} \citep{Rcpp}. The object model, the C++
...
Certificates are the check that survives leaving our test suite. \texttt{verify\_assignment()} and \texttt{verify\_flow()} run in \(O(nm)\) and
...
article. Each caches its CSV under \texttt{paper/} and re-measures only when that file is moved aside. \texttt{paper/run\_bench\_suite.sh} runs them in order on one machine,
...
\caption{\label{tab:implicit-table}One-to-one optimal Mahalanobis matching by memory mode, wall-clock seconds on a single core of an Apple M4 Pro, the fastest of three repetitions with the arms interleaved. Graph built is the arcs the implicit loop ended up holding as a share of the complete problem's. Distances is its evaluations over all rounds as a multiple of the complete pair count, so a pair priced twice counts twice. All three arms were timed in one session; the dense arm was run at the four sizes where its pairing can be compared against the loop's, and the scaling table below carries dense timings at the two largest sizes. Every cell returned the same total distance.}
\centering
\begin{tabular}[t]{lrrrrrr}
\toprule
Problem size & dense & lazy & implicit & Graph built & Distances & Rounds\\
\midrule
167 + 333 & 10 ms & 5 ms & 6 ms & 16.22\% & 2.00 & 2\\
667 + 1,333 & 109 ms & 45 ms & 53 ms & 4.95\% & 2.00 & 2\\
1,667 + 3,333 & 678 ms & 331 ms & 306 ms & 2.16\% & 1.96 & 2\\
3,333 + 6,667 & 2.78 s & 1.55 s & 1.15 s & 1.17\% & 1.88 & 2\\
6,667 + 13,333 & not run & 6.25 s & 4.16 s & 0.63\% & 1.71 & 2\\
\addlinespace
16,667 + 33,333 & not run & 61 s & 21.9 s & 0.29\% & 1.42 & 2\\
\bottomrule
\end{tabular}
\end{table}
...
materialised matrix takes 2.78 seconds
under Jonker-Volgenant and 3.66
under the flow solver the restricted master belongs to, against 1.15 for the loop, so what the loop
...
time. The factors are covariate counts from 2 to
32, seven point clouds, three caliper settings including the
...
distance is nearly equal. Across the 20 cells that grid holds, the loop settled in
...
8x on the displaced treated group, and none at all on
the heavy-tailed cloud, where the two paths are within measurement of each other.
...
At 6,667 + 13,333 the dense path peaks 6452 MB
above that floor, the lazy path 33 MB and the
loop 138 MB, against
27418 MB for \CRANpkg{optmatch} and
8643 MB for \CRANpkg{MatchIt}. Two readings follow. The loop is not the smallest of the three: the lazy path
holds 33 MB against the loop's 138 MB, since it stores no graph at all
...
for 1094 MB on a matrix whose entries
occupy 711 MB, so holding it costs 1.5 times its cells and solving over it costs 9.1 times them. That gap is preparation copies
...
peak of the solve at 12 times them, and \texttt{memory\_mode\ =\ "auto"}
compares the second against available memory. Neither multiplier is fitted to one cell: across 5,000, 10,000 and 20,000 units the
solve peaked at 9.5, 7.2 and 9.1 times the cell bytes and the matrix at
1.9, 1.7 and 1.5 times them, and both defaults sit above every one of those. Both ratios are read from 5,000
units up: at 2,000 the matrix holds
7 MB of cells
...
5,000 units, reached 9.5 times the cells
against the 12 the guard assumes, leaving
113 MB of headroom.
...
0 relative, so the path reproduces
...
\(n = 5000\) within a reasonable budget. We measured all timings in this section on 2026-09-15 under R 4.5.3 with couplr 1.7.1 at commit d76fe36,
...
figure is a solver's \texttt{method} name: JV \texttt{jv}, SAP \texttt{sap}, SAP-D \texttt{sap\_dense},
LAPMOD \texttt{lapmod}, Auction \texttt{auction}, Auction-GS \texttt{auction\_gs}, Auction-S
\texttt{auction\_scaled}, CSA \texttt{csa}, CS-flow \texttt{csflow}, Push-R \texttt{push\_relabel}, Cycle-C
\texttt{cycle\_cancel}, Net-S \texttt{network\_simplex}, SSAP-B \texttt{ssap\_bucket}, Gabow-T
\texttt{gabow\_tarjan}, Ramshaw \texttt{ramshaw\_tarjan}, Hungarian \texttt{hungarian}, Munkres
\texttt{munkres}, HK-01 \texttt{hk01} and Brute-F \texttt{bruteforce}.
...
crossing and 45 the large-tier subset. We generate each cell as
several independent instances and time each instance several times, and every
solver in the panel sees the same instance. We time the solver \texttt{"auto"} selects
...
\midrule
no earlier rule applies & jv & 144 & 83 & 1.02x & 18.00x\\
no cost scale to exploit & hk01 & 45 & 44 & 0.99x & 3.22x\\
\bottomrule
\end{tabular}
\end{table}

Over the 189 cells the dispatcher picked the fastest solver of the
panel in 127 of them, at a median 1.02x of the
cell's best time. Where a cell reads below \texttt{1.00x} the two sides are separate
timings of the same solver, and the lowest in the grid is
0.91x. The two rules that were
removed can be read off the same grid. Over the 96 cells with
...
half the entries forbidden, 1.02x. Timed against the
solver each rule named rather than against the panel's best, \texttt{sap} takes a
median 1.31x of the default's time on the wide cells and \texttt{lapmod}
1.12x of it on the sparse ones, so in each case the rule was naming the
slower of the two.

Where the default is behind, it is behind in one cost regime. Over the
18 cells whose finite costs are heavily tied it takes a median
5.09x of the fastest solver in the cell and as much as
18.00x; over the remaining 126 cells of the
rule the median is 1.02x and the worst
4.34x. The solvers that win the tied cells are
\texttt{network\_simplex} in 7 of them and
\texttt{auction\_scaled} in 4, both of them the
...
\texttt{memory\_mode\ =\ "dense"}. We generate each size as several independent
...
\caption{\label{tab:scaling}One-to-one optimal Mahalanobis matching, wall-clock time by problem size. Treated to control ratio 1:2, eight covariates, pooled within-group covariance, single core with single-threaded BLAS on an Apple M4 Pro. Each cell is the median over independently generated instances of that size, with the interquartile range across instances in brackets; an instance's time is the fastest of its repetitions, taken with the packages interleaved.  Fewer than four instances back the bracket at 10,000 (3), 20,000 (3) and 50,000 (3), so there it is the spread of those runs rather than a quartile estimate. The optmatch option max.problem.size was set to Inf for the two largest sizes. int overflow marks an integer-size overflow inside the optmatch back end reached through MatchIt; timeout marks a run exceeding the 600-second cap.}
\centering
\begin{tabular}[t]{lrrr}
\toprule
Problem size & couplr & optmatch & MatchIt\\
\midrule
167 + 333 & 11 ms (11-11) & 98 ms (96-98) & 115 ms (115-116)\\
667 + 1,333 & 114 ms (109-116) & 1.64 s (1.56-1.65) & 2.03 s (1.93-2.08)\\
1,667 + 3,333 & 679 ms (677-724) & 12 s (12-12.1) & 14.8 s (14.7-15.1)\\
3,333 + 6,667 & 2.93 s (2.85-3.02) & 58.5 s (57.7-58.5) & 71.2 s (70.2-71.3)\\
6,667 + 13,333 & 12.6 s (11.6-12.8) & 289 s (282-293) & int overflow\\
\addlinespace
16,667 + 33,333 & 78.8 s (78.7-79.5) & timeout & timeout\\
\bottomrule
\end{tabular}
\end{table}
...
\(n = 500\) and 23 times faster at
\(n = 20{,}000\), where it finishes in
12.6 seconds against
4.8 minutes.
...
problems under \texttt{memory\_mode\ =\ "lazy"} take a median
...
\texttt{optmatch::pairmatch()} and MatchIt's optimal method route it through
\texttt{fullmatch()}, and it exposes nineteen solvers where optmatch exposes five. The
...
\item
  ...
  Jonker-Volgenant, at a median 1.02x of the cell's
  fastest solver over most of the regime grid. Where the finite costs are
  heavily tied it takes a median 5.09x and as much as
  18.00x, behind \texttt{network\_simplex} and
  \texttt{auction\_scaled}. A rule for tied costs, fixed in advance and
