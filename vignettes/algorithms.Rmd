...
```{r animate-hungarian, eval = has_widgets}
set.seed(1)
...
```{r animate-auction, eval = has_widgets}
set.seed(2)
...
`"sap_dense"` runs the same successive-shortest-path search with the opposite
storage assumption. Its Dijkstra step picks the next column by scanning every
unfinalized one instead of pulling from a heap, which costs $O(m)$ per
extraction and $O(n m^2)$ over a full solve. On a dense cost matrix, where
every column is a candidate at every step, the scan does the work a heap would
do anyway without paying for the heap.
