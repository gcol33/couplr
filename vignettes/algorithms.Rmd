...
ggplot() +
  ...
  geom_label(data = edges[edges$type == "considering", ],
             aes(x = (from_x + to_x)/2, y = (from_y + to_y)/2 - 0.15, label = label),
             fill = "#dfd9d2", color = "#3e3f3a", size = 3.5) +
  # Source nodes
...
# Network nodes
nodes <- data.frame(
  label = c("Source", "R1", "R2", "R3", "C1", "C2", "C3", "Sink"),
...
ggplot() +
  ...
  # Layer labels
  annotate("text", x = 0, y = 0.3, label = "Supply = n", size = 3.5, color = "#3e3f3a") +
  annotate("text", x = 1.5, y = 0.3, label = "Rows", size = 3.5, color = "#f47c3c") +
  annotate("text", x = 3.5, y = 0.3, label = "Columns", size = 3.5, color = "#29abe0") +
  annotate("text", x = 5, y = 0.3, label = "Demand = n", size = 3.5, color = "#3e3f3a") +
