// Test suite for pure C++ Brute-Force solver
...
TEST_CASE("Bruteforce solver - basic square matrices", "[bruteforce][basic]") {
    SECTION("3x3 matrix") {
        auto cost = make_cost({
            {1, 2, 3},
            {4, 5, 6},
        ...
        auto result = lap::solve_bruteforce(cost, false);
...
TEST_CASE("Bruteforce solver - rectangular matrices", "[bruteforce][rectangular]") {
...
TEST_CASE("Bruteforce solver - maximization", "[bruteforce][maximize]") {
    SECTION("3x3 maximize") {
        ...
        auto result = lap::solve_bruteforce(cost, true);
...
TEST_CASE("Bruteforce solver - forbidden edges", "[bruteforce][forbidden]") {
...
TEST_CASE("Bruteforce solver - empty matrix", "[bruteforce][edge]") {
...
TEST_CASE("Bruteforce solver - infeasible cases", "[bruteforce][infeasible]") {
    SECTION("more rows than cols throws") {
        ...
        REQUIRE_THROWS_AS(lap::solve_bruteforce(cost, false), lap::DimensionException);
    }
    ...
    SECTION("row with all forbidden throws") {
        ...
        REQUIRE_THROWS_AS(lap::solve_bruteforce(cost, false), lap::InfeasibleException);
    }
}

TEST_CASE("Bruteforce solver - size limit", "[bruteforce][limit]") {
    SECTION("n=8 should work") {
        lap::CostMatrix cost(8, 8);
        for (int i = 0; i < 8; ++i) {
            for (int j = 0; j < 8; ++j) {
                cost.at(i, j) = static_cast<double>(i * 8 + j);
            }
        }

        auto result = lap::solve_bruteforce(cost, false);
        REQUIRE(result.status == "optimal");
        REQUIRE(result.n_matched() == 8);
    }
}

TEST_CASE("Bruteforce solver - assignment validity", "[bruteforce][validity]") {
