...
#> employed: std diff = 0.365
```

Substantial baseline imbalance exists: treatment group is younger (-0.9
SD), more educated (+0.7 SD), and has higher prior earnings (+0.6 SD).
...
MatchIt (Ho et al., 2011) is the most widely used matching package in R.
It emphasizes propensity score methods and provides a unified interface
to multiple matching algorithms.

**Key difference**: MatchIt typically matches on estimated propensity
scores (a single summary measure), while couplr matches directly on
covariates (multivariate distance).
...
optmatch (Hansen & Klopfer, 2006) specializes in optimal matching using
...
Both packages use optimal assignment algorithms, so they should achieve
similar total distances. The key differences are in:
...
designmatch (Zubizarreta et al., 2018) uses mixed-integer programming to
find matches satisfying explicit balance constraints. Rather than
minimizing distance, it finds feasible matches that meet user-specified
balance criteria.

**Key difference**: designmatch optimizes for balance constraints
directly; couplr minimizes total distance (balance is a consequence, not
a constraint).
...
couplr doesn’t constrain balance directly but achieves balance through
...
The Matching package (Sekhon, 2011) provides genetic matching and
multivariate matching with automatic balance optimization.

**Key difference**: Matching uses genetic algorithms to find weights
that optimize balance; couplr uses fixed weights (after scaling) and
optimizes assignment.
...
**Note**: couplr’s greedy matching is fastest for large problems. For
optimal matching, optmatch’s RELAX-IV is competitive. couplr offers more
algorithm choices for different scenarios.

### Memory Usage

| Package | Memory Model |
|----|----|
| couplr | Full distance matrix by default; `memory_mode = "lazy"` computes distances on demand for `method = "jv"`/`"auction"` |
| MatchIt | Propensity scores + sparse distance |
| optmatch | Sparse distance matrices |
| designmatch | Full distance + constraint matrices |

couplr’s greedy mode solves the same full matrix faster; it does not
reduce memory use. `memory_mode = "lazy"` (with
`method = "jv"`/`"auction"`) does reduce memory use, by computing each
pairwise distance from the underlying feature data instead of allocating
the full matrix; `memory_mode = "auto"` (the default) switches to it
automatically for large problems. Blocking further shrinks the matrix
allocated per block under either memory mode.
...
- Zubizarreta, J. R., Kilcioglu, C., & Vielma, J. P. (2018).
  designmatch: Matched samples that are balanced and representative by
  design. R package.
  [CRAN](https://CRAN.R-project.org/package=designmatch)
