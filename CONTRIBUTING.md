...
to the **couplr** project!

This document provides guidelines for contributing to couplr—its codebase and documentation.
...
couplr uses **testthat** for testing.
...
- **Export pattern**: Do NOT put `[[Rcpp::export]]` in subdirectory files. Instead, create `_impl` functions in subdirectories and add exported wrappers in `src/rcpp_interface.cpp`.
...
- Method/algorithm used if relevant
- R and operating system version
- Toolchain info if relevant (e.g., Rtools on Windows)

---

By contributing to couplr, you agree that your code is released under the same license as the package.
