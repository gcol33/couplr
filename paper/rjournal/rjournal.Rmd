...
abstract: >
  ...
  addresses both. Every matching design whose answer is an optimisation compiles
  into one internal flow model, so node potentials come back from all of them and
  ...
  the same loop traces a matching frontier over a swept constraint far more
  cheaply than solving each point cold. A data-frame interface, nineteen
...
design in the package whose answer is an optimisation compiles into it, so one
...
rather than what the matrix occupies, at a multiplier read off the
measurements in the Performance section, and compares that against available
system memory read per platform. If the estimate exceeds half of what is
available, `"auto"` switches to the lazy path where one exists and warns where
...
cell's best time. The two rules that were removed can be read off the same
table. Over the `r nrow(reg_rect)` cells with at least three columns per row,
where the rectangular rule used to fire, the default takes a median
...
not fit. `estimate_dense_matrix_mb()` estimates the matrix and
`estimate_dense_solve_mb()` the peak of the solve, at a multiplier read off
these measurements, and `memory_mode = "auto"` compares the second against
available memory. Every arm and size is in the supplementary material.
...
matrix. Three design consequences follow. Every design whose answer is an
optimisation compiles into one flow model, so node potentials come back from
all of them. Those potentials make a solution checkable
...
v_j$, and Mahalanobis distance is Euclidean after whitening, so one tree
serves both metrics. Where such a bound prunes, the pruning is itself the
certificate. Calipers are the case for it, being axis-aligned boxes in the
original covariates and where matching users actually work.
