// Test suite for pure C++ Successive Shortest Path (SSP) solver
...
#include "solvers/solve_ssp_pure.h"
...
TEST_CASE("SSP solver - basic square matrices", "[ssp][basic]") {
    SECTION("3x3 matrix") {
        auto cost = make_cost({
            {1, 2, 3},
            {4, 5, 6},
        ...
        auto result = lap::solve_ssp(cost, false);
...
TEST_CASE("SSP solver - rectangular matrices", "[ssp][rectangular]") {
...
TEST_CASE("SSP solver - maximization", "[ssp][maximize]") {
    SECTION("3x3 maximize") {
        ...
        auto result = lap::solve_ssp(cost, true);
...
TEST_CASE("SSP solver - forbidden edges", "[ssp][forbidden]") {
...
TEST_CASE("SSP solver - empty matrix", "[ssp][edge]") {
...
TEST_CASE("SSP solver - infeasible cases", "[ssp][infeasible]") {
    SECTION("row with all forbidden throws") {
        ...
        REQUIRE_THROWS_AS(lap::solve_ssp(cost, false), lap::InfeasibleException);
    }
}

TEST_CASE("SSP solver - assignment validity", "[ssp][validity]") {
