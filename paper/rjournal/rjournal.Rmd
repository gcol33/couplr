...
so no unit is discarded. The default solves the corresponding minimum-cost flow
problem with Johnson potentials [@Johnson1977]; a two-pass greedy heuristic is
reached by asking for it, under `method = "greedy"`. `cem_match()` implements coarsened exact matching
...
restricted graph needs a statement about the complete one to know when to stop.
...
`r sprintf("%.2f", 1 / median(pth$wall_speedup))` of solving each point independently, and
...
- The lazy cost path covers one-to-one matching without replacement under
  ...
  above one, `cardinality_match()` and a user-supplied distance function, which
...
design the flow model compiles, and with it to `full_match()`. A bound tighter
