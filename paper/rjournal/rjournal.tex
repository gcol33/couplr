...
\midrule
167 + 333 & 11 ms & 3 ms & 6 ms & 16.22\% & 2.00 & 2\\
667 + 1,333 & 106 ms & 46 ms & 54 ms & 4.95\% & 2.00 & 2\\
1,667 + 3,333 & 672 ms & 338 ms & 312 ms & 2.16\% & 1.96 & 2\\
3,333 + 6,667 & 2.78 s & 1.55 s & 1.17 s & 1.17\% & 1.88 & 2\\
6,667 + 13,333 & not run & 6.25 s & 4.23 s & 0.63\% & 1.71 & 2\\
\addlinespace
16,667 + 33,333 & not run & 60.8 s & 22.2 s & 0.29\% & 1.42 & 2\\
\bottomrule
\end{tabular}
\end{table}
...
under Jonker-Volgenant and 3.7
under the flow solver the restricted master belongs to, against 1.17 for the loop, so what the loop
...
10x on the displaced treated group, and none at all on
...
At 6,667 + 13,333 the dense path peaks 6150 MB
above that floor, the lazy path 23 MB and the
loop 129 MB, against
27419 MB for \CRANpkg{optmatch} and
8640 MB for \CRANpkg{MatchIt}. Two readings follow. The loop is not the smallest of the three: the lazy path
holds 23 MB against the loop's 129 MB, since it stores no graph at all
...
occupy 711 MB, so holding it costs 1.5 times its cells and solving over it costs 8.6 times them. That gap is preparation copies
...
solve peaked at 10.5, 7.2 and 8.6 times the cell bytes and the matrix at
1.8, 1.7 and 1.5 times them, and both defaults sit above every one of those. Both ratios are read from 5,000
...
5,000 units, reached 10.5 times the cells
...
\midrule
167 + 333 & 0.061 s & 0.142 s & 2.33x & 2.70x & 22 / 40\\
667 + 1,333 & 0.581 s & 1.16 s & 2.00x & 2.01x & 23 / 40\\
1,667 + 3,333 & 3.27 s & 6.73 s & 2.06x & 2.06x & 21 / 40\\
6,667 + 13,333 & 46.3 s & 93.1 s & 2.01x & 2.01x & 21 / 40\\
\bottomrule
\end{tabular}
\end{table}
...
\(n = 5000\) within a reasonable budget. We measured all timings in this section on 2026-09-14 under R 4.5.3 with couplr 1.7.1 at commit 2b850df,
...
\midrule
no earlier rule applies & jv & 144 & 70 & 1.03x & 10.55x\\
no cost scale to exploit & hk01 & 45 & 44 & 0.99x & 2.25x\\
\bottomrule
\end{tabular}
\end{table}

Over the 189 cells the dispatcher picked the fastest solver of the
panel in 114 of them, at a median 1.02x of the
...
1.03x; over the 96 cells with more than
half the entries forbidden, 1.03x. Timed against the
...
3.97x of the fastest solver in the cell and as much as
10.55x; over the remaining 126 cells of the
rule the median is 1.03x and the worst
4.22x. The solvers that win the tied cells are
\texttt{network\_simplex} in 10 of them and
...
\midrule
167 + 333 & 10 ms (10-10) & 97 ms (96-97) & 118 ms (116-118)\\
667 + 1,333 & 114 ms (114-117) & 1.6 s (1.53-1.63) & 2.05 s (2.01-2.08)\\
1,667 + 3,333 & 683 ms (680-747) & 12 s (11.9-12) & 14.9 s (14.9-15)\\
3,333 + 6,667 & 2.87 s (2.86-2.96) & 58.3 s (57.6-58.4) & 71.3 s (70.4-71.3)\\
6,667 + 13,333 & 12.9 s (11.7-13.3) & 285 s (280-291) & int overflow\\
\addlinespace
16,667 + 33,333 & 78.7 s (77.3-79.1) & timeout & timeout\\
\bottomrule
\end{tabular}
\end{table}

The margin widens with problem size. couplr is about
10 times faster than \CRANpkg{optmatch} at
\(n = 500\) and 22 times faster at
\(n = 20{,}000\), where it finishes in
12.9 seconds against
4.7 minutes.
...
\item
  ...
  above one, \texttt{cardinality\_match()} and a user-supplied distance function build
  the dense matrix.
\item
...
\item
  ...
  Jonker-Volgenant, at a median 1.03x of the cell's
  fastest solver over most of the regime grid. Where the finite costs are
  heavily tied it takes a median 3.97x and as much as
  10.55x, behind \texttt{network\_simplex} and
  \texttt{auction\_scaled}. Fitting a rule to this grid would score it on
