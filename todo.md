# couplr Algorithm Integration TODO

## Completed
- [x] Network Simplex implementation (all 20 tests passing)
- [x] Orlin-Ahuja solver
- [x] Push-Relabel solver
- [x] Ramshaw-Tarjan rectangular solver
- [x] Assignment duals extraction
- [x] All algorithms added to `assignment()` main routine
- [x] Run `devtools::document()` to regenerate `.Rd` documentation
- [x] Update `_pkgdown.yml` with new functions (assignment_duals, bottleneck_assignment, sinkhorn, sinkhorn_to_assignment)
- [x] Rebuild pkgdown website using `build_site.R`

## Remaining Tasks
- [x] Commit all changes (b9a3094)
