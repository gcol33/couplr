...
**Key difference**: optmatch pioneered optimal full matching via network flow, and its `fullmatch()` admits one-to-many and many-to-one groups in the same solution. couplr's `full_match()` is the narrower variable-ratio design: every group holds one unit of the smaller side and several of the larger. Where the two shapes can be mixed, optmatch's optimum can be strictly cheaper. couplr's contribution here is one-to-one matching with 20+ algorithm choices and a checkable certificate.
...
3. **Variable-ratio grouping**: `full_match()` in couplr and `fullmatch()` in optmatch. optmatch solves the wider full-matching problem, which also admits many-to-one groups; couplr fixes the group centres to the smaller side.
...
|---------|-------------|--------|
| **Objective** | Satisfy balance constraints | Minimize total distance (`match_couples()`), or maximize matched cardinality under balance constraints (`cardinality_match()`) |
...
- Fine or refined balance is the requirement, and a certificate of the largest
  balanced sample is wanted (`cardinality_match()`)
...
couplr's greedy mode solves the same full matrix faster; it does not reduce
memory use. `memory_mode = "lazy"` (with `method = "jv"`/`"auction"`) does
reduce memory use, by computing each pairwise distance from the underlying
feature data instead of allocating the full matrix; `memory_mode = "auto"`
(the default) switches to it automatically for large problems. Blocking
further shrinks the matrix allocated per block under either memory mode.
