...
costs. No rule in the table reads how tied the costs are and
none was added for it: fitting one to this grid would score it on the
...
couplr has four limitations:

- The lazy and implicit cost paths cover the built-in metrics. A user-supplied
  distance function builds the dense matrix.
...
- The dispatcher reads two properties and otherwise defaults to
  ...
  `` `r names(tied_win)[2]` ``. Fitting a rule to this grid would score it on
  the measurements that chose it, so we leave the default as it is.
