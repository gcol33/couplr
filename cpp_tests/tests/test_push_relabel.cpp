// Test suite for pure C++ Push-Relabel solver
...
TEST_CASE("Push-Relabel solver - basic square matrices", "[push_relabel][basic]") {
    SECTION("3x3 matrix") {
        auto cost = make_cost({
            {1, 2, 3},
            {4, 5, 6},
        ...
        auto result = lap::solve_push_relabel(cost, false);
...
TEST_CASE("Push-Relabel solver - rectangular matrices", "[push_relabel][rectangular]") {
...
TEST_CASE("Push-Relabel solver - maximization", "[push_relabel][maximize]") {
    SECTION("3x3 maximize") {
        ...
        auto result = lap::solve_push_relabel(cost, true);
...
TEST_CASE("Push-Relabel solver - forbidden edges", "[push_relabel][forbidden]") {
...
TEST_CASE("Push-Relabel solver - empty matrix", "[push_relabel][edge]") {
...
TEST_CASE("Push-Relabel solver - infeasible cases", "[push_relabel][infeasible]") {
    SECTION("more rows than cols throws") {
        ...
        REQUIRE_THROWS_AS(lap::solve_push_relabel(cost, false), lap::DimensionException);
    }
    ...
    SECTION("row with all forbidden throws") {
        ...
        REQUIRE_THROWS_AS(lap::solve_push_relabel(cost, false), lap::InfeasibleException);
    }
}

TEST_CASE("Push-Relabel solver - assignment validity", "[push_relabel][validity]") {
