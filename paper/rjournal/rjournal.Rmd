...
times them. Both ratios are read from `r format(min(mem_sizes), big.mark = ",")`
...
the peak measures there is the session rather than the solve, and both
multipliers are exceeded on it. Both defaults bound every one of the sizes
they are read against. The tightest solve peak, at `r mem_tight_n` units,
reached `r sprintf("%.1f", mem_tight_x)` times the cells against the
`r solve_factor` the guard assumes, leaving `r sprintf("%.0f", mem_tight_mb)` MB
of headroom there. The guard exists to refuse a solve that will not fit rather
than to predict where a peak lands, so the multiplier is set above the ratios it
is read against.
...
cost matrix. Every flow-representable design compiles into one flow model, so
node potentials come back from all of them, those potentials turn a reported
status into a certificate, and the certificate is what makes it safe to solve a
problem that was never built.

What that architecture buys is measurable. At `r ilab[6]` the restricted graph
settled at `r share[6]` of the complete pair count and returned a certified
optimum, and over the `r nrow(regime)` cells of the regime grid the dispatcher
ran at a median `r x(median(regime$ratio))` of the cell's fastest solver.
