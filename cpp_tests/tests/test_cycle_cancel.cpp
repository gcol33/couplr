// Test suite for pure C++ Cycle Canceling solver
...
#include "solvers/solve_cycle_cancel_pure.h"
...
TEST_CASE("Cycle Cancel solver - basic square matrices", "[cycle_cancel][basic]") {
    SECTION("3x3 matrix") {
        auto cost = make_cost({
            {1, 2, 3},
            {4, 5, 6},
        ...
        auto result = lap::solve_cycle_cancel(cost, false);
...
TEST_CASE("Cycle Cancel solver - rectangular matrices", "[cycle_cancel][rectangular]") {
...
TEST_CASE("Cycle Cancel solver - maximization", "[cycle_cancel][maximize]") {
    SECTION("3x3 maximize") {
        ...
        auto result = lap::solve_cycle_cancel(cost, true);
...
TEST_CASE("Cycle Cancel solver - forbidden edges", "[cycle_cancel][forbidden]") {
...
TEST_CASE("Cycle Cancel solver - empty matrix", "[cycle_cancel][edge]") {
    SECTION("empty matrix throws") {
        // Cycle Cancel throws on empty matrix rather than returning empty result
        lap::CostMatrix cost(0, 0);

        REQUIRE_THROWS(lap::solve_cycle_cancel(cost, false));
    }
}

TEST_CASE("Cycle Cancel solver - infeasible cases", "[cycle_cancel][infeasible]") {
    SECTION("row with all forbidden throws") {
        ...
        REQUIRE_THROWS_AS(lap::solve_cycle_cancel(cost, false), lap::InfeasibleException);
    }
}

TEST_CASE("Cycle Cancel solver - assignment validity", "[cycle_cancel][validity]") {
