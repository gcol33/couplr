...
Matching is a fundamental technique for creating comparable groups in observational studies, treatment effect estimation, and experimental design. Unlike random assignment, observational data often contain systematic differences between groups that can confound causal inference. Matching addresses this by pairing similar units across groups based on observed characteristics (covariates).

This vignette demonstrates production-ready matching workflows using `couplr`'s high-level matching functions introduced in v1.0.0. These tools handle the complete pipeline from raw data to publication-ready results, including automatic preprocessing, balance assessment, and diagnostic reporting.
...
- Treatment effect estimation in epidemiology and health services research
- Program evaluation in economics and social sciences
- Creating matched cohorts for comparative effectiveness studies
- Balancing baseline characteristics in quasi-experimental designs
- Sample matching for survey weighting

**Related vignettes:**

- For algorithm details: `vignette("algorithms")`
- For LAP foundations: `vignette("getting-started")`
- For balance interpretation: `vignette("balance-diagnostics")` (upcoming)
...
balance <- balance_diagnostics(
  ...
  left_data = matched_left,
  right_data = matched_right,
  vars = c("age", "income")
)
...
# Compute pre-match balance
pre_match <- balance_diagnostics(
  match_result = NULL,  # No matching
  left_data = left_data,
...
balance_comparison <- bind_rows(
  pre_match$var_stats %>% mutate(when = "Before"),
  balance$var_stats %>% mutate(when = "After")
)
...
job_match <- match_couples(
  ...
  auto_scale = TRUE,
  scale = "robust",
  max_distance = 0.5,  # Caliper at 0.5 pooled SD
  return_diagnostics = TRUE
)
...
job_balance <- balance_diagnostics(
  ...
  left_data = matched_treated,
  right_data = matched_control,
  vars = c("age", "education_years", "prior_earnings", "employed")
)
...
**Greedy matching complexity:** O(n² log n) for sorted, O(n²) for row-best
...
This vignette demonstrated production-ready matching workflows with `couplr`:

**Key Takeaways:**

- **Automatic preprocessing** handles scaling, categorical variables, and data quality issues
- **Optimal matching** provides globally best solution for n < 5,000
- **Greedy matching** offers fast approximation for large datasets
- **Calipers** ensure minimum match quality by excluding poor pairs
- **Blocking** enables exact balance on key variables and improves scalability
- **Balance diagnostics** assess match quality via standardized differences, variance ratios, and KS tests
- **Real-world workflow** integrates matching into treatment effect estimation pipeline

**Recommended Workflow:**

1. Explore data, identify covariates to balance
2. Apply `match_couples()` with `auto_scale = TRUE`
3. Assess balance with `balance_diagnostics()`
4. Refine if needed (add caliper, blocking, or try greedy)
5. Report balance tables and treatment effects

**Learn more:**

- Algorithm details: `vignette("algorithms")`
- Balance interpretation: `vignette("balance-diagnostics")` (upcoming)
- Large-scale matching: `vignette("large-scale-matching")` (upcoming)
- Function documentation: `?match_couples`, `?greedy_couples`, `?balance_diagnostics`
- Causal inference methods: `vignette("observational-studies")` (upcoming)
